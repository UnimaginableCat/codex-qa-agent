...
All workspace tooling must run through the workspace-root venv, not a target project venv under `code/`. Before the first `tools.generation`, `tools.scenario_runner`, `tools/api`, `tools/db`, or helper CLI command, resolve an interpreter located directly under the workspace root, such as `.venv314/Scripts/python.exe`, `.venv/Scripts/python.exe`, `.venv314/bin/python`, or `.venv/bin/python`. Do not probe `python -m ...`, `python3 -m ...`, `py -m ...`, `uv run`, or `code/<project>/.venv*/...` before checking the workspace venv. The readiness probe must call the public workspace CLI needed for the task, for example `<candidate> -m tools.generation.cli --help`, `<candidate> -m tools.scenario_runner.batch_cli --help`, or `<candidate> -m tools.scenario_runner.cli --help`; internal helper modules such as `tools.generation.cli_authoring_bundle`, `tools.generation.cli_authoring`, `tools.generation.cli_generation_run`, `tools.generation.cli_review`, or `tools.generation.cli_rendering` are not readiness probes. Store the first candidate that passes the public CLI probe and reuse that exact interpreter for later commands. Do not hard-code a different candidate such as `.venv314/bin/python` after a probe loop found another path. If a probe returns no stdout, no `usage:` text, and no explicit status evidence, treat readiness as unknown and rerun the public CLI probe instead of concluding that the workspace venv is missing. If no workspace-root venv can run the requested workspace CLI, report tooling readiness as `BLOCKED`; use a project venv, `uv run`, system `python`, `python3`, or `py` only after explicit user authorization for that fallback.
