...
class ApiConnectivityClassificationTests(unittest.TestCase):
    ...
    def test_binary_response_body_is_omitted_but_result_stays_structured(self) -> None:
        ...
        self.assertEqual(result.details["response"].content_length_bytes, 15)
        self.assertEqual(result.details["response"].body_content_type, "application/pdf")
        self.assertTrue(result.details["response"].body_is_binary)

    def test_empty_binary_response_preserves_zero_length_metadata(self) -> None:
        result = ApiRequestService(
            session=_ResponseSession(200, content=b"", content_type="application/pdf"),
            resolver=_passing_resolver,
            system_resolver_diagnostics=False,
        ).execute(
            self._step(),
        ...
        self.assertEqual(result.status, StepStatus.PASS)
        self.assertEqual(result.details["response"].http_status, 200)
        self.assertIn("bytes=0", result.details["response"].body)
        self.assertEqual(result.details["response"].content_length_bytes, 0)
        self.assertTrue(result.details["response"].body_is_binary)
