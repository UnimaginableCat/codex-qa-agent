...
class ScenarioRenderingTests(unittest.TestCase):
    ...
    def test_renderer_does_not_require_db_for_explicit_read_only_post_workflow(self) -> None:
        render_result = DraftScenarioRenderer().render(
            _plan(
                [
                    PlannedTestCase(
                        case_id="tc-calculate-001",
                        title="Calculate template",
                        objective="Verify read-only calculate endpoint.",
                        metadata={"state_change": "read_only"},
                        workflow_steps=[
                            PlannedWorkflowStep(
                                step_type="api",
                                title="Discover template",
                                route=PlannedRouteIntent(http_method="GET", endpoint_path="/price-lists/{{price_list_id}}"),
                                expected_outcomes=["HTTP 200"],
                            ),
                            PlannedWorkflowStep(
                                step_type="api",
                                ...
                                expected_outcomes=["HTTP 200", "response contains field `template_items.0.total_cost`"],
                            ),
                        ],
                    )
                ]
        ...
        self.assertEqual(len(render_result.draft_set.drafts), 1)
        draft = render_result.draft_set.drafts[0]
        self.assertIn("DB verification required: no.", draft.markdown)
        self.assertIn("No persistence expected: case is explicitly authored as read-only.", draft.markdown)
