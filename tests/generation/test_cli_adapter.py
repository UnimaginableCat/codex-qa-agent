...
""",
    ...
    def test_validate_operation_inventory_blocks_parent_scoped_collection_one_row_verifier(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            operation_inventory_path = root / "operation-inventory.yaml"
            operation_inventory_path.write_text(
...
db_verifications:
  - entity: price_list_template_variable
    operation: verify_thickness_variable
    scoped_by: template_id
    sql: >
      SELECT id, code
      FROM price_list_pricelisttemplatevariable
      WHERE template_id = :template_id AND code = 'thickness_cm'
    params:
      template_id: "{{template_id}}"
    expected_outcomes:
      - one row exists
...
""",
    ...
    def test_validate_operation_inventory_blocks_action_like_route_without_method_evidence(self) -> None:
        for route_path in (
            "/api/price_list/{{price_list_id}}/search/",
            "/api/users/search-users/",
            "/api/price_list/{{price_list_id}}/export_excel/",
            "/api/reports/download-report/",
        ):
            with self.subTest(route_path=route_path), TemporaryDirectory() as tmp:
                root = Path(tmp)
                (root / "entity-inventory.yaml").write_text(
                    """version: 1
source_id: price-list-api
...
""",
    ...
    def test_validate_operation_inventory_allows_action_like_route_with_method_evidence(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "entity-inventory.yaml").write_text(
                """version: 1
...
""",
    ...
    def test_validate_operation_inventory_rejects_route_method_evidence_without_declared_method(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "entity-inventory.yaml").write_text(
                """version: 1
...
routes:
  - method: PATCH
    ...
    method_evidence:
      source_ref: code/demo/views.py
      evidence: PriceListUpdateView extends AbstractUpdateView and is_partial_update returns True.
db_verifications: []
metadata:
  contracts:
    routes:
...
""",
    ...
    def test_validate_operation_inventory_treats_string_false_method_evidence_contract_as_disabled(self) -> None:
...
metadata:
  contracts:
    routes:
      method_evidence_required: {raw_value}
""",
                        encoding="utf-8",
                    )
    ...
    def test_validate_operation_inventory_malformed_false_does_not_disable_action_like_method_evidence(self) -> None:
        for raw_value in ("maybe", "null", '""'):
            with self.subTest(raw_value=raw_value), TemporaryDirectory() as tmp:
                root = Path(tmp)
                (root / "entity-inventory.yaml").write_text(
                    """version: 1
source_id: price-list-api
