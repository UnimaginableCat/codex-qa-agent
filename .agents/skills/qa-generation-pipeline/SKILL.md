...
- Authoring validation reports `authoring_request_body_field_evidence_required`: return to `agent-plan-authoring` and replace generic serializer/schema prose with field-specific `request_body_evidence.required`, `fields`, `properties`, or `request_constraints` matching every authored top-level body key. Do not satisfy this with `source_ref` paths or unrelated metadata strings that happen to contain a field name.
