...
4. write `authoring-plan.yaml`
   ...
   - for permission negative/default coverage, declare the claim in `case.metadata.coverage_claims.permissions` with `actor`, `permission`, `expected_state`, and `expected_result` when the role/right being denied matters. Strict permission gates use structured claims and explicit fixture/setup/baseline contracts; they do not infer permission intent from prose, actor metadata, route/body context, or a bare `403` status.
