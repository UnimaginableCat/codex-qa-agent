...
4. write `authoring-plan.yaml`
   ...
   - for permission negative/default coverage, declare the claim in `case.metadata.coverage_claims.permissions` with `actor`, `permission`, `expected_state`, and `expected_result` when the role/right being denied matters. Strict permission gates use structured claims, actor metadata, status, and route/body context rather than requiring redundant role prose in the objective.
