...
- For search/list visibility cases with `requires_non_empty_result: true`, prove that the response is non-empty before asserting masking. Use setup that creates/discovers a matching record, an explicit stable non-empty fixture contract, or a supported oracle check such as `response categories length >= 1` / `response contains field \`categories.0.positions.0.id\``. `response JSON exists` is not proof that masking was exercised.
