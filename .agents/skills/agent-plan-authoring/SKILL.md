...
- Never infer the HTTP method for PDF, Excel, export, download, report, calculate, or action-like endpoints from the route name. Record the method from router/controller evidence. For operation inventories where route accuracy is critical, set `metadata.contracts.routes.method_evidence_required: true` and add structured `method_evidence` on each route, for example `source_ref: code/<project>/.../urls.py` plus `evidence: <controller method/action proving POST>`. Free-form strings such as `search view is POST` or dictionaries that only repeat `method: POST` are not evidence. If the method cannot be proven, keep the case deferred/open rather than guessing `GET` or `POST` for an action-like path.
...
Open questions that say something must be confirmed "before promoting", "before running", or "before execution" are blocking authoring questions. Resolve them, mark the affected cases deferred, or move the item to non-blocking notes before downstream generation; do not promote scenarios while such questions remain.
