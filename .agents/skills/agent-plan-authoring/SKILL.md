...
- When a workflow grants/revokes permissions for a role/principal and then executes as that actor, bind the permission subject to the actor profile. Do not grant the first captured user/member/principal id from a management list and then execute as a named actor unless metadata or setup proves that captured identity belongs to that actor credentials. This also applies when the permission effect subject is a placeholder such as `{{partner_company_member_guid}}`; the validator treats it as unbound unless the plan proves which actor owns that captured principal. Prefer capturing the current actor principal id/guid, using an actor-owned env fixture with `metadata.identity_resolution` justification, or adding structured `metadata.identity_resolution.actor_binding`.
