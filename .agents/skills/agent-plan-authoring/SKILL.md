...
- Never infer the HTTP method for PDF, Excel, export, download, report, calculate, or action-like endpoints from the route name. Record the method from router/controller evidence. For operation inventories where route accuracy is critical, set `metadata.contracts.routes.method_evidence_required: true` and add `method_evidence` on each route, for example the URL mapping plus the controller method/action source. If the method cannot be proven, keep the case deferred/open rather than guessing `GET` for a download-like path.
