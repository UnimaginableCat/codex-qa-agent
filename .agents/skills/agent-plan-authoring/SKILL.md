...
- For action-like `POST`/`PUT`/`PATCH` cases with authored request bodies, request body evidence must name the exact top-level fields being sent. Generic text such as `uses serializer` is not enough. Prefer structured evidence such as `request_body_evidence.required: [templates, quantity]`, `fields: [...]`, or `request_constraints` entries for each authored body key; otherwise the case can pass authoring with `template_id/count` while the real serializer expects `templates/quantity` and fail at runtime with HTTP 400.
