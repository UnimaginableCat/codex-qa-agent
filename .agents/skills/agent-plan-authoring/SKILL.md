...
- For search/list visibility cases with `requires_non_empty_result: true`, prove that matching fixture data exists before asserting masking. Use setup that creates/discovers a matching record or an explicit structured contract such as `metadata.data_contract.non_empty_paths` / `metadata.fixture_contract.non_empty_paths` for the exact indexed response path. A plain oracle check such as `response items length >= 1` or `response contains field \`items.0.id\`` only turns missing fixture data into runtime FAIL; it does not prove readiness.
- When a workflow grants/revokes permissions for a role/principal and then executes as that actor, bind the permission subject to the actor profile. Do not grant the first captured user/member/principal id from a management list and then execute as a named actor unless metadata or setup proves that captured identity belongs to that actor credentials. Prefer capturing the current actor principal id/guid, using an actor-owned env fixture with `metadata.identity_resolution` justification, or adding structured `metadata.identity_resolution.actor_binding`.
...
For permissions negative/default cases, do not assume a role fixture currently lacks overrides just because the role normally defaults to denied. Batch runs can mutate shared permission fixtures, and QA envs may already have `can_edit=true` or `can_create=true`. Prefer a structured claim such as `metadata.coverage_claims.permissions: {actor: partner, permission: can_create, expected_state: false, expected_result: denied}` so validation does not depend on prose wording. Make the case self-contained by resetting/revoking the relevant permission in setup, or document a stable no-override fixture contract such as `metadata.stable_permission_fixture`. Even with a stable fixture contract, add a baseline setup/read step that checks current effective permissions or the override row before the denial/default assertion; otherwise a previous run can leave the partner already granted. When a plan enables `metadata.contracts.permissions.negative_cases_require_state_setup: true`, negative/default cases without setup or fixture contract block validation. When it also enables `metadata.contracts.permissions.negative_cases_require_baseline_check: true`, stable fixture cases without a baseline check block validation.
