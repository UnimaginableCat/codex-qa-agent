...
Before running any `tools.generation` CLI command, resolve and use the workspace-root venv interpreter. Check only root-level venv candidates such as `.venv314/Scripts/python.exe`, `.venv/Scripts/python.exe`, `.venv314/bin/python`, and `.venv/bin/python`. Do not use `code/<project>/.venv*`, `python`, `python3`, `py`, or `uv run` as probes before checking the workspace venv. Probe readiness with the public generation CLI only: `<candidate> -m tools.generation.cli --help`. Do not use internal helper modules such as `tools.generation.cli_authoring_bundle`, `tools.generation.cli_authoring`, `tools.generation.cli_generation_run`, `tools.generation.cli_review`, or `tools.generation.cli_rendering` as readiness checks; some can exit with no output and prove nothing about the real CLI. Store the first candidate that passes the public CLI probe and reuse that exact `<venv-python>` for all later generation commands. Do not hard-code another candidate after the probe loop. A root venv created by tools such as `uv` may have a `python` executable that resolves to an external base interpreter path; do not declare it non-workspace solely from `readlink`/resolved executable location. Trust the workspace CLI guard result, which validates the active venv prefix. If a probe returns no stdout or no `usage:`/status evidence, treat readiness as unknown and rerun the public CLI probe instead of inferring success or absence. If no workspace-root venv interpreter can run the generation CLI, report authoring tooling readiness as `BLOCKED`; use any non-workspace interpreter only after explicit user authorization.
...
- For action-like `POST`/`PUT`/`PATCH` cases with authored request bodies, request body evidence must name the exact top-level fields being sent. Generic text such as `uses serializer` is not enough, and metadata strings such as `source_ref: serializers/user.py` do not count as field evidence. Prefer structured evidence such as `request_body_evidence.required: [templates, quantity]`, `fields: [...]`, or `request_constraints` entries for each authored body key; otherwise the case can pass authoring with `template_id/count` while the real serializer expects `templates/quantity` and fail at runtime with HTTP 400.
...
- For visibility or masking cases, declare the claim structurally in `metadata.coverage_claims.visibility`; do not rely on objective wording to activate strict coverage gates. Include the field names, exact response paths, and whether a non-empty collection is required, for example:
  ...
  ```
- For visibility or masking cases, assert the actual field behavior when the response is JSON, using the exact JSON path proven by serializer/API evidence. Do not assert root-level `response \`price\` = \`null\`` or `response \`cost_price\` = \`null\`` for price-list detail/search/template responses unless source evidence proves those fields are at response root. For nested indexed paths, prove every indexed collection level: `response \`categories\` length >= \`1\`` only proves `categories.0`, not `categories.0.positions.0`; add `response \`categories.0.positions\` length >= \`1\`` or an equivalent structured `metadata.fixture_contract.non_empty_paths` / executable setup contract before asserting `response \`categories.0.positions.0.cost_price\` = \`null\``. Prose-only preconditions are not review readiness proof.
