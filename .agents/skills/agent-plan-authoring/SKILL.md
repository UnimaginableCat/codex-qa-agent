...
Make the case self-contained by resetting/revoking the relevant permission in setup, or document a stable no-override fixture contract such as `metadata.stable_permission_fixture`. Even with a stable fixture contract, add a baseline setup/read step that checks current effective permissions or the override row before the denial/default assertion; otherwise a previous run can leave the partner already granted. A baseline cannot be satisfied by a prose string or by saying another case covers it, for example `covered by pl-perm-003`; generated scenarios must be independent. If baseline evidence is stored in metadata, use a structured object with `verified: true`, an evidence field such as `setup_operation`, `route`, `query`, `source_ref`, or `assertions`, and an explicit false/absent `expected_state`. When a plan enables `metadata.contracts.permissions.negative_cases_require_state_setup: true`, structured negative/default permission claims without setup or fixture contract block validation. When it also enables `metadata.contracts.permissions.negative_cases_require_baseline_check: true`, stable fixture cases without structured baseline evidence block validation.
