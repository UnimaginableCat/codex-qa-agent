...
- For all workspace CLIs, including `tools.generation`, `tools.scenario_runner`, `tools/api`, and `tools/db`, resolve the workspace-root venv before the first command. Check only root-level candidates such as `.venv314/Scripts/python.exe`, `.venv/Scripts/python.exe`, `.venv314/bin/python`, and `.venv/bin/python`. Do not use `code/<project>/.venv*`, `python -m ...`, `python3 -m ...`, `py -m ...`, or `uv run` as probes before the workspace venv check. Probe the public CLI required by the task, for example `<candidate> -m tools.generation.cli --help` or `<candidate> -m tools.scenario_runner.batch_cli --help`; do not use internal helper modules as readiness checks. Store the first passing candidate and reuse that exact interpreter instead of hard-coding another candidate later. A root venv may use a symlinked executable that resolves outside the workspace; if the workspace CLI guard accepts the active venv prefix, continue instead of reporting readiness `BLOCKED` from resolved path alone. If a probe has no stdout and no explicit `usage:`/status evidence, readiness is unknown, not PASS and not proof that the venv is absent. Use a project venv, `uv run`, system `python`, `python3`, or `py` only after the workspace-root venv fails and the user explicitly authorizes that fallback.
