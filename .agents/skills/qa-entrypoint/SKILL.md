...
- For all workspace CLIs, apply the `AGENTS.md` workspace interpreter rule: probe the public CLI required by the task, store the first passing workspace-root interpreter, and reuse it. Do not use internal helper modules, project venvs, system Python, `py`, or `uv run` unless the root venv fails and the user authorizes the fallback.
- For `tools.generation.cli` gates, require explicit machine-readable or text status evidence before continuing. Empty stdout, truncated output without `Status: ...`, or an unreadable JSON payload is not a successful gate; rerun the generation CLI with JSON/text output or read the persisted generation artifact before reporting `PASS`. Do not add generation-only flags such as `--output-format` to `tools.scenario_runner.*` commands; runner evidence comes from runner stdout and persisted runner artifacts.
- Treat preflight/readiness blockers as setup blockers, not as authorization to alter test intent. Do not remove env-backed variables, request fields, setup, assertions, captures, or DB checks just to turn `BLOCKED` into `PASS`; report the missing readiness item and change coverage only with explicit user intent or direct implementation evidence.
