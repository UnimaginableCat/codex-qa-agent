...
Resolve `<venv-python>` before the first command. Use a workspace-root venv interpreter such as `.venv314/Scripts/python.exe`, `.venv/Scripts/python.exe`, `.venv314/bin/python`, or `.venv/bin/python`; do not use target project venvs under `code/<project>`, `python`, `python3`, `py`, or `uv run` as probes before checking the workspace venv. Probe readiness with `<candidate> -m tools.generation.cli --help`, store the first passing candidate, and reuse that exact path for all downstream generation commands. Do not probe internal helper modules such as `tools.generation.cli_authoring_bundle`, and do not switch to a hard-coded candidate after a probe loop. A workspace venv may resolve to an external base interpreter path when it is symlinked by `uv`; rely on whether the workspace CLI guard accepts the active venv prefix, not on resolved executable path alone. If a probe returns no stdout or no `usage:`/status evidence, treat readiness as unknown and rerun the public CLI probe. If no workspace-root venv can run the generation CLI, stop as tooling `BLOCKED`; use any non-workspace interpreter only after explicit user authorization.
...
- Treat runtime HTTP 400 on action-like authored request bodies as a source authoring defect unless implementation evidence proves the product rejected valid input. Return to `agent-plan-authoring` and make request body evidence field-specific and schema-backed in the same evidence item. Evidence must explicitly declare schema role, such as `source_role: request_schema`, or provide an inline schema-shaped contract such as `schema`, `properties`, or `request_body_schema`; file paths, service-layer prose, separate generic schema markers, and non-schema `fields: [...]` are not enough to prove that fields such as `template_id`/`count` match the real request payload.
...
- Treat runner failures whose expectation text contains prose operators such as `contains ... with`, `items include`, or `contains variable/item with` as source authoring defects, not product failures. Return to `authoring-plan.yaml` and express the behavior as supported JSON paths, array membership support, or DB verification; do not weaken the assertion to `response body exists`.
- Treat permission negative/default failures where the actor actually has `can_edit` or `can_create` as fixture-state authoring defects by default. Repair the source case with a structured `metadata.coverage_claims.permissions` contract plus self-contained revoke/reset setup, or an executable baseline setup step that reads/verifies the effective permission state immediately before the denied/default action. Metadata-only `stable_permission_fixture` / `permission_baseline_checked` notes are not enough for runnable scenarios unless an explicit permissions contract opts into accepting metadata baselines. Do not rely on objective/title wording, actor metadata, or a bare `403` response to identify permission intent.
