...
- Treat runtime HTTP 400 on action-like authored request bodies as a source authoring defect unless implementation evidence proves the product rejected valid input. Return to `agent-plan-authoring` and make request body evidence field-specific and schema-backed. Evidence must explicitly declare schema role, such as `source_role: request_schema`, or provide an inline schema-shaped contract such as `schema`, `properties`, or `request_body_schema`; file paths, service-layer prose, and `fields: [...]` alone are not enough to prove that fields such as `template_id`/`count` match the real request payload.
