...
- Treat review/runtime evidence of empty search/list visibility results as an authoring defect when the source case declares `metadata.coverage_claims.visibility.requires_non_empty_result: true`. Repair the source with data-creating/discovery setup or an explicit structured fixture/data contract such as `metadata.data_contract.non_empty_paths` plus provenance (`source`, `source_ref`, `evidence`, `setup_operation`, or DB verification) for the exact indexed response path; do not treat a length assertion or bare `non_empty_paths` in the oracle/metadata as readiness proof.
